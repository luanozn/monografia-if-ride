...
A utilização de um autorizador JWT no Amazon API Gateway, mesmo com a presença de mecanismos de autenticação e autorização na camada de aplicação, fundamenta-se no princípio de defesa em profundidade. A validação antecipada do token permite que requisições inválidas ou malformadas sejam bloqueadas antes de alcançarem a instância EC2, evitando chamadas desnecessárias que podem causar sobrecarga na máquina, visto que em tipos de instância com menor poder de processamento o acúmulo de requisições simultâneas pode acarretar em lentidão ou até mesmo na derrubada do sistema.

Essa estratégia é particularmente relevante em cenários de alta volumetria de requisições ou potenciais ataques de negação de serviço em nível de aplicação, nos quais o aumento do processamento da EC2 poderia resultar em lentidão significativa e degradação do serviço. Ao concentrar a validação inicial no API Gateway, que opera de forma altamente escalável e com menor custo, a arquitetura torna-se mais eficiente, resiliente e economicamente sustentável.

Após a autenticação, as requisições válidas são direcionadas à aplicação hospedada no AWS Elastic Compute Cloud (EC2), responsável pela execução da lógica de negócio. O uso de um serviço como o EC2, totalmente em nuvem e sob demanda possibilita que o usuário desfrute de um estilo de cobrança específico em que somente serão cobradas as instâncias ativas, contribuindo para a otimização de custos, uma vez que o usuário não dispõe de um custo inicial relacionado à infraestrutura, somente às instâncias que provisionar.
